...
VoxelModel::VoxelModel(const std::filesystem::path &path, const int resolution, const bool print_info) {
...
void VoxelModel::print_model_info(void) const {
  for (const auto view : this->views) {
    std::cout << "[View " << view.name << "]" << std::endl
      ...
      << "Vx = [" << view.vy.x << ", " << view.vy.y << ", " << view.vy.z << "]" << std::endl
      << "Vx = [" << view.vz.x << ", " << view.vz.y << ", " << view.vz.z << "]" << std::endl
      << std::endl;
  }
}
...
void VoxelModel::initial_reconstruction(void) {
...
void VoxelModel::model_refinement(void) {
...
void VoxelModel::surface_generation(void) {
  // Calcular las dimensiones de los cubos
...
void VoxelModel::additional_info(void) const {
